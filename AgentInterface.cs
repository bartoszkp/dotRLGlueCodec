...
namespace DotRLGlueCodec
{
    ...
    {
        void AgentInit(string taskSpecification);
        Action AgentStart(Observation observation);
        Action AgentStep(double reward, Observation observation);
        void AgentEnd(double reward);
        void AgentCleanup();
        string AgentMessage(string message);
    }
}
